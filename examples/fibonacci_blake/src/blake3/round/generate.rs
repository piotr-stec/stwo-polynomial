...
impl TraceGeneratorRow<'_> {
    fn append_felt(&mut self, val: u32x16) {
        self.generate.trace[self.col_index].data[self.vec_row] =
    ...
    fn generate(&mut self, mut v: [u32x16; 16], m: [u32x16; 16]) {
        ...
        chain![input_v.iter(), v.iter(), m.iter()]
            .flat_map(to_felts)
            .enumerate()
            .for_each(|(i, felt)| self.generate.round_lookup[i].data[self.vec_row] = felt);
    }
    ...
    fn xor(&mut self, w: u32, a: u32x16, b: u32x16) -> u32x16 {
        let c = a ^ b;
        self.append_felt(c);
        if self.generate.xor_lookups.len() <= self.xor_lookups_index {
            self.generate.xor_lookups.push((
                w,
                std::array::from_fn(|_| unsafe {
                    BaseColumn::uninitialized(1 << self.generate.log_size)
                }),
            ));
        }
        self.generate.xor_lookups[self.xor_lookups_index].1[0].data[self.vec_row] =
            unsafe { PackedBaseField::from_simd_unchecked(a) };
        self.generate.xor_lookups[self.xor_lookups_index].1[1].data[self.vec_row] =
            unsafe { PackedBaseField::from_simd_unchecked(b) };
        self.generate.xor_lookups[self.xor_lookups_index].1[2].data[self.vec_row] =
